...
- For automated quality review after generation, the report-quality-auditor agent is available in the foremanos-field plugin
...
**Inline QA Checks (from report-quality-auditor methodology):**
If the foremanos-field plugin is not installed, apply these checks directly:
- **W1 — Schedule Narrative Accuracy**: Compare schedule narrative claims against actual milestone movement in `schedule.json`. Flag discrepancies between reported progress and scheduled dates.
- **W2 — Crew Trend Consistency**: Verify headcount trends in weekly summary match aggregated daily report headcounts. Flag >15% discrepancy.
- **W3 — Open Items Accuracy**: Cross-check open RFI/submittal/CO counts against `rfi-log.json`, `submittal-log.json`, `change-order-log.json`. Flag mismatches.
- **W4 — Weather Impact Reconciliation**: Verify weather delay hours claimed in weekly summary match daily weather/delay entries. Flag unsubstantiated delay claims.
- **W5 — Photo Coverage**: Check that photos represent multiple days and activity types. Flag if all photos are from one day or one area.
- **W6 — Distribution List Verification**: Verify distribution list matches contract requirements from `specs-quality.json`.
