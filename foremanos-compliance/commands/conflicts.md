...
This command uses the conflict-detection-agent methodology (canonical agent in foremanos-intel plugin). The 7-step detection methodology is applied directly:

1. **Plan vs. Specification conflicts** (CDR-05 through CDR-10): Compare dimensions, materials, equipment specs between plans-spatial.json and specs-quality.json
2. **Specification vs. Schedule conflicts** (CDR-14 through CDR-17): Verify schedule durations account for spec requirements (cure times, lead times, weather restrictions)
3. **Drawing vs. Drawing conflicts** (CDR-01 through CDR-04, CDR-21 through CDR-23): Cross-check dimensions, grid lines, and details across sheets in plans-spatial.json
4. **Cost vs. Scope conflicts** (CDR-18 through CDR-20): Compare cost-data.json budget lines against scope defined in plans-spatial.json and specs-quality.json
5. **Dual-Source conflicts** (CDR-24, CDR-25): Reconcile data extracted from multiple document sources (e.g., mechanical schedule on plans vs. spec section requirements)
6. **Temporal/Revision conflicts**: Check for data based on superseded document versions
7. **Categorize and prioritize**: Assign severity (CRITICAL/MAJOR/MINOR) based on cost impact, safety risk, and schedule effect

For the full conflict-detection-agent with autonomous monitoring capabilities, install the foremanos-intel plugin.
