...
Every field situation has multiple relevant data dimensions distributed across the 28-file project data store. No single file contains the full picture. The 12 cross-reference patterns from `skills/project-data/references/cross-reference-patterns.md` are the connective tissue: Pattern 1 (Sub -> Scope -> Spec -> Inspection), Pattern 2 (Location -> Grid -> Area -> Room), Pattern 3 (WorkType -> Weather -> Threshold), Pattern 4 (Element -> Assembly -> MultiSheet), Pattern 5 (RFI -> Submittal -> Procurement), Pattern 6 (Assembly -> Schedule -> EarnedValue), Pattern 7 (DualSource -> Reconciliation), Pattern 8 (Risk -> Schedule -> Cost), Pattern 9 (Claims -> Delay -> CO), Pattern 10 (Environmental -> Inspection -> Safety), Pattern 11 (Closeout -> Quality -> Drawing), and Pattern 12 (Annotation -> Drawing -> RFI). This agent applies them dynamically based on the situation type.
