...
Read the project-data skill at `${CLAUDE_PLUGIN_ROOT}/skills/project-data/SKILL.md` before proceeding. Also read the dashboard-intelligence-analyst agent at `${CLAUDE_PLUGIN_ROOT}/agents/dashboard-intelligence-analyst.md` for daily pulse briefing methodology. If the foremanos-planning plugin is installed, the deadline-sentinel agent provides comprehensive deadline monitoring across all project data. Reference `${CLAUDE_PLUGIN_ROOT}/skills/project-data/references/masterformat-reference.md` for MasterFormat best practices when enriching today's activities. For field-specific knowledge (concrete operations, steel erection, earthwork, BMPs, equipment), install the foremanos-field plugin which includes 21 trade-specific reference guides. If the project is in closeout phase and the foremanos-compliance plugin is installed, also consult the closeout-commissioning skill for closeout deadlines and commissioning status.
